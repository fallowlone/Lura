...
/// Координаты X и ширина берутся из taffy (он корректно считает flexbox/grid).
/// Координата Y вычисляется через cursor_y — taffy Y полностью игнорируется.
///
/// Разрывы страниц: если блок не вмещается → новая страница.

use super::layout::{LayoutContent, LayoutNodeIdx, LayoutTree,
                    A4_HEIGHT_PT, A4_WIDTH_PT, PAGE_MARGIN_PT, MM_TO_PT, CONTENT_WIDTH_PT};
use super::styles::{BoxKind, Color, FontStyle, FontWeight};
...
impl<'a> Paginator<'a> {
    fn new(layout: &'a LayoutTree, styled: &'a super::arena::DocumentArena) -> Self {
        Self { layout, styled, pages: vec![Page::new()], cursor_y: CONTENT_TOP }
    }
    ...
    /// Размещает узел, возвращает высоту, которую он занял (без внешних отступов).
    fn place_node(&mut self, node_idx: LayoutNodeIdx) -> f32 {
        ...
        let consumed = match node.content.clone() {
            LayoutContent::Text(text) => {
                ...
                let lines = break_text(&text, width, styles.font_size, styles.line_height);
                let block_h = text_block_height(&lines);

                // Page break если блок не вмещается
    ...
    fn place_table(&mut self, table_idx: LayoutNodeIdx, row_indices: Vec<LayoutNodeIdx>) -> f32 {
        ...
        // Рамка таблицы (высота пересчитается в конце — используем оценку)
        ...
        for (row_num, row_idx) in row_indices.into_iter().enumerate() {
            ...
            // Вычисляем высоту строки = max высота ячейки
            let row_height = cell_indices.iter().map(|&ci| {
                ...
                match &cell.content {
                    LayoutContent::Text(t) => {
                        ...
                        let lines = break_text(t, w, cell_styles.font_size, cell_styles.line_height);
            ...
            }).fold(0.0f32, f32::max).max(14.0); // минимум 14pt высота строки

            // Page break для строки целиком
            if self.cursor_y + row_height > CONTENT_BOTTOM && self.cursor_y > CONTENT_TOP {
                self.new_page();
            }

            // Фон header-строки (первая строка таблицы)
            if row_num == 0 {
                self.push_cmd(DrawCommand::Rect {
                    x: table_x,
                    y: row_start_y,
                    w: table_w,
                    h: row_height,
                    fill: Some(Color::from_hex(0xF5F5F5)),
                    stroke: None,
                    stroke_width: 0.0,
                });
            }

            // Рисуем ячейки в текущей строке
            for &cell_idx in &cell_indices {
                ...
                if let LayoutContent::Text(text) = &cell.content {
                    let w = cell.width.max(1.0);
                    let lines = break_text(text, w, cell_styles.font_size, cell_styles.line_height);
                    for (i, line) in lines.iter().enumerate() {
                        let y = self.cursor_y + padding_top + cell_styles.font_size
                        ...
                        self.push_cmd(DrawCommand::Text {
                            ...
                            bold:   cell_styles.font_weight == FontWeight::Bold,
    ...
    fn estimate_height(&self, node_idx: LayoutNodeIdx) -> f32 {
        ...
        match &node.content {
            LayoutContent::Text(t) => {
                ...
                let lines = break_text(t, w, styles.font_size, styles.line_height);
                text_block_height(&lines)
            }
...
pub fn paginate(layout: &LayoutTree, styled: &super::arena::DocumentArena) -> PageTree {
    ...
    for &root_idx in &layout.roots {
        ...
        // PAGE — пропускаем саму PAGE-обёртку, обрабатываем детей напрямую
        match &node.content {
            LayoutContent::Children(children) => {
